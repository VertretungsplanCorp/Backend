...
def scrape():
    ...
    for index in range(len(SUBDIRS)):
        ...
        while True:
            ...
            url = f"{
                BASE}/{SUBDIRS[index]}/{filename}" if SUBDIRS[index] else f"{BASE}/{filename}"
...
conn = psycopg2.connect(
    ...
    user=os.getenv('DB_USER'),  # liest den Wert von DB_USER aus der .env-Datei
